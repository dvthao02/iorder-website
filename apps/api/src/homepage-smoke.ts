...
const input = { title: 'Trang chủ', seoTitle: 'iOrder smoke homepage', seoDescription: 'Homepage smoke test', canonicalUrl: null, blocks: [
  { type: 'hero', isEnabled: true, data: { eyebrow: 'iOrder', title: 'Published hero title', description: 'Homepage smoke description', imageMediaId: null, primaryLabel: 'Liên hệ', primaryUrl: '/lien-he', secondaryLabel: null, secondaryUrl: null } },
  { type: 'article_list', isEnabled: true, data: { heading: 'Bài viết mới', postType: 'all', limit: 3 } },
] }
...
try {
  ...
  const [user] = await database.db.insert(users).values({ username, fullName: 'Homepage Smoke User', passwordHash: await hashPassword(password), status: 'active' }).returning({ id: users.id })
  ...
  const header = login.headers['set-cookie']; const raw = Array.isArray(header) ? header[0] : header; const cookie = raw?.split(';', 1)[0]
  ...
  const draftChanged = structuredClone(input); draftChanged.blocks[0]!.data.title = 'Unpublished changed title'
  ...
  await app.close()
  if (pageId) { await database.db.delete(auditLogs).where(eq(auditLogs.entityId, pageId)); await database.db.delete(pages).where(eq(pages.id, pageId)) }
  if (userId) { await database.db.delete(auditLogs).where(eq(auditLogs.userId, userId)); await database.db.delete(users).where(eq(users.id, userId)) }
