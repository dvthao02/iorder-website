...
try {
  ...
  const created = await app.inject({ method: 'POST', url: '/api/admin/posts', headers: { cookie }, payload: draftInput })
  ...
  const duplicate = await app.inject({ method: 'POST', url: '/api/admin/posts', headers: { cookie }, payload: draftInput })
  ...
  if (updated.statusCode !== 200 || !updated.body.includes('Updated posts smoke draft')) throw new Error('Post update failed')
  ...
  if (publicPost.statusCode !== 200 || !publicPost.body.includes('Updated posts smoke draft')) throw new Error('Published post was not returned by public API')

  const publicList = await app.inject({ method: 'GET', url: '/api/public/posts?type=news' })
  if (publicList.statusCode !== 200 || !publicList.body.includes(postId)) throw new Error('Published post was not returned by public list')
  ...
  const [revisionTotal] = await database.db.select({ value: count() }).from(postRevisions).where(eq(postRevisions.postId, postId))
  if ((revisionTotal?.value ?? 0) < 4) throw new Error('Post revisions were not recorded for create, update, publish, and archive')
