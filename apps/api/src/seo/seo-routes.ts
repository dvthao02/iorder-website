...
export function registerSeoRoutes(app: FastifyInstance, options: { db: CmsDatabase; publicOrigin: string }) {
  ...
  app.get('/sitemap.xml', async (_request, reply) => {
    ...
    const [postRows, offeringRows] = await Promise.all([
      options.db.select({ slug: posts.slug, updatedAt: posts.updatedAt }).from(posts)
        .where(and(eq(posts.status, 'published'), isNull(posts.deletedAt), or(isNull(posts.publishedAt), lte(posts.publishedAt, now)))),
      options.db.select({ type: offerings.type, slug: offerings.slug, updatedAt: offerings.updatedAt }).from(offerings)
