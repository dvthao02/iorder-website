...
function newBlock(type: HomepageBlock['type'], mediaId = ''): HomepageBlock {
  ...
  if (type === 'home_features') return { type, isEnabled: false, appearance, data: { eyebrow: 'NỀN TẢNG', heading: 'Một nền tảng cho toàn bộ vận hành cửa hàng', intro: null, items: [{ title: 'Tính năng mới', description: 'Mô tả tính năng', href: null, mediaId: null }] } }
...
export function HomepageEditor() {
  ...
  return <section className="admin-card content-manager">
    ...
    <div className="fixed-editor-layout no-preview">
      ...
      {/* home_features */}
      {block.type === 'home_features' ? <div className="nested-editor"><div className="nested-heading"><strong>Các tính năng</strong><button type="button" onClick={() => updateItems(index, [...block.data.items, { title: 'Mục mới', description: 'Mô tả', href: null, mediaId: null }])}>+ Thêm mục</button></div>{block.data.items.map((item, itemIndex) => <div className="nested-row" key={itemIndex}><input value={item.title} onChange={(e) => updateItems(index, block.data.items.map((value, i) => i === itemIndex ? { ...value, title: e.target.value } : value))} /><input value={item.description} onChange={(e) => updateItems(index, block.data.items.map((value, i) => i === itemIndex ? { ...value, description: e.target.value } : value))} /><input placeholder="Link (không bắt buộc)" value={item.href ?? ''} onChange={(e) => updateItems(index, block.data.items.map((value, i) => i === itemIndex ? { ...value, href: e.target.value || null } : value))} /><select value={item.mediaId ?? ''} onChange={(e) => updateItems(index, block.data.items.map((value, i) => i === itemIndex ? { ...value, mediaId: e.target.value || null } : value))}><option value="">— Dùng icon mặc định —</option>{images.map((img) => <option key={img.id} value={img.id}>{img.originalName}</option>)}</select><button type="button" onClick={() => updateItems(index, block.data.items.filter((_, i) => i !== itemIndex))}>Xóa</button></div>)}</div> : null}
