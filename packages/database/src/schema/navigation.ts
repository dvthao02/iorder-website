...
export const menus = pgTable('menus', {
  ...
  uniqueIndex('menus_location_unique').on(table.location),
])
...
export const linkGroups = pgTable('link_groups', {
  ...
  uniqueIndex('link_groups_code_unique').on(table.code),
])

export const contentLinks = pgTable('content_links', {
  id: uuid('id').defaultRandom().primaryKey(),
  groupId: uuid('group_id').notNull().references(() => linkGroups.id, { onDelete: 'cascade' }),
  label: varchar('label', { length: 180 }).notNull(),
  url: text('url').notNull(),
